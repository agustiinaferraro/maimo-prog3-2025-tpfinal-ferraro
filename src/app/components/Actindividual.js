...
const Actindividual = () => {
  ...
  const enteringIdx = leaving !== null && nextIdxRef.current !== null ? nextIdxRef.current : currentIndex;

  const renderPanel = (act, displayIdx, anim = "") => (
            ...
            <div className="w-full lg:w-1/2 flex flex-col items-center py-12 flex-shrink-0">
              ...
              <div className="flex items-center gap-4 mt-4 text-white">
                ...
                <span className="text-white/60 text-base font-medium">
                  {displayIdx + 1} / {actividades.length}
                </span>
                <button
                  onClick={() => goTo("right")}
                  disabled={busy.current}
  ...
  return (
    <div className="relative w-full py-10 overflow-hidden" style={{ minHeight: "500px" }}>
      {leaving !== null && renderPanel(actividades[leaving], leaving, leaveAnim)}
      {renderPanel(actividades[enteringIdx], enteringIdx, leaving !== null ? enterAnim : "")}
    </div>
  );
};
